...
def admin_delete_parent(parent_id):
    conn = get_db()
    conn.execute("DELETE FROM feedback WHERE appointment_id IN (SELECT id FROM appointments WHERE parent_id=?)", (parent_id,))
    conn.execute("DELETE FROM lessons WHERE appointment_id IN (SELECT id FROM appointments WHERE parent_id=?)", (parent_id,))
    conn.execute("DELETE FROM contracts WHERE appointment_id IN (SELECT id FROM appointments WHERE parent_id=?)", (parent_id,))
    conn.execute("DELETE FROM late_payments WHERE parent_id=?", (parent_id,))
    conn.execute("DELETE FROM bought_leads WHERE (buyer_type='parent' AND buyer_id=?) OR (target_type='parent' AND target_id=?)", (parent_id, parent_id))
    conn.execute("DELETE FROM appointments WHERE parent_id=?", (parent_id,))
    conn.execute("DELETE FROM verification_docs WHERE user_type='parent' AND user_id=?", (parent_id,))
...
def admin_delete_teacher(teacher_id):
    conn = get_db()
    conn.execute("DELETE FROM feedback WHERE appointment_id IN (SELECT id FROM appointments WHERE teacher_id=?)", (teacher_id,))
    conn.execute("DELETE FROM lessons WHERE appointment_id IN (SELECT id FROM appointments WHERE teacher_id=?)", (teacher_id,))
    conn.execute("DELETE FROM contracts WHERE appointment_id IN (SELECT id FROM appointments WHERE teacher_id=?)", (teacher_id,))
    conn.execute("DELETE FROM earnings WHERE teacher_id=?", (teacher_id,))
    conn.execute("DELETE FROM late_payments WHERE teacher_id=?", (teacher_id,))
    conn.execute("DELETE FROM coupons WHERE teacher_id=?", (teacher_id,))
    conn.execute("DELETE FROM bought_leads WHERE (buyer_type='teacher' AND buyer_id=?) OR (target_type='teacher' AND target_id=?)", (teacher_id, teacher_id))
    conn.execute("DELETE FROM appointments WHERE teacher_id=?", (teacher_id,))
    conn.execute("DELETE FROM teacher_fees WHERE teacher_id=?", (teacher_id,))
    conn.execute("DELETE FROM withdrawals WHERE teacher_id=?", (teacher_id,))
